...
RegisterCommand("vote", function(source, args)
    ...
    PerformHttpRequest("http://194.87.183.24:8081/api/votes/get", function(err, text, headers)
...
RegisterServerEvent('fivemil-votes:claim', function(data)
    ...
    PerformHttpRequest("http://194.87.183.24:8081/api/votes/claim", function(err, text, headers) 
        if text == 'ok' then
            if Prize(src) then
